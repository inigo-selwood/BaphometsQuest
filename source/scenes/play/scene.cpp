...
void Scene::input(const SDL_Event &event) {
    ...
    if(event.key.keysym.sym == SDLK_RETURN
        ...
        const auto textbox = this->getChild("screen-layer.textbox");

        if(textbox->isActive() && !this->selectionActive) {
            this->hideTextbox();
            return;
        }
    }
...
void Scene::showTextbox(const std::string &text) {
    const auto textbox = this->getChild("screen-layer.textbox");
...
void Scene::hideTextbox() {
    const auto textbox = this->getChild("screen-layer.textbox");

    textbox->setProperty("active", false);
}

bool Scene::isSelectionKey(const SDL_Event &event) const {
    return event.key.keysym.sym == SDLK_LSHIFT
        || event.key.keysym.sym == SDLK_RSHIFT;
}

SDL_Point Scene::getSelectionMovement(const SDL_Event &event) const {
    const int step =
        this->getChild("world-layer.world.player")->getProperty<int>("step");
...
void Scene::moveSelectionCursor(SDL_Point movement) {
...
void Scene::setSelectionActive(bool active) {
...
void Scene::updateSelectionMode() {
...
std::string Scene::getSelectModeName() const {
...
bool Scene::isCloseRangeSelectMode() const {
    return this->selectMode == SelectMode::Sword
        || this->selectMode == SelectMode::Axe;
}

SDL_Point Scene::clampSelectionPosition(SDL_Point position) const {
    const auto player = this->getChild("world-layer.world.player");
...
std::shared_ptr<Scenes::Play::Components::PauseMenu>
Scene::getPauseMenu() const {
