...
std::vector<Node::PropertyInfo> Node::getProperties() const {
    std::vector<PropertyInfo> propertyInformation;
    propertyInformation.reserve(this->properties.size());

    for(const auto &[name, property] : this->properties) {
        propertyInformation.push_back(
            PropertyInfo{
                name,
                property->owner,
                property->type,
...
void Node::exitTree() {
    ...
    this->onExitTree();
    spdlog::trace("Exited tree {} ({})", this->getName(), this->getTypeName());
    this->inTree = false;
}
...
Node::PropertyBindingBase &Node::getPropertyBinding(const std::string &name) {
    const auto propertyIterator = this->properties.find(name);

    if(propertyIterator == this->properties.end()) {
        throw std::runtime_error(
            "Unknown property '" + name + "' on node '" + this->getName()
            + "'."
        );
    }
...
const Node::PropertyBindingBase &
Node::getPropertyBinding(const std::string &name) const {
    const auto propertyIterator = this->properties.find(name);
...
void Node::setPropertyFromString(
    const std::string &name,
...
void setNodePropertyFromString(
    ...
    node.setPropertyFromString(name, value);
}
